"""
A simple cocos based program. A blue ball (player) must collect red balls. Arrow keys move around the board.
"""
import cocos
...
class Actor(cocos.sprite.Sprite):
    def __init__(self, x, y, color):
        super(Actor, self).__init__('images/ball.png', color=color)
        self.position = pos = eu.Vector2(x, y)
        self.cshape = cm.CircleShape(pos, self.width/2)


class MainLayer(cocos.layer.Layer):
    """ Defines a custom layer that inherits from cocos and runs parent init. """
